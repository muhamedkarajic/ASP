...
template <class Type>
class ListSequence : public List<Type>
{
  private:
    Type *array;
    int counter;
  ...
  public:
    ...
    ListSequence(Type *array, int size, int max = 10) : ListSequence(max)
    {
        for (int i = 0; i < size; i++)
            add(array[i]);
    }

    ListSequence(List<Type> *list, int max = 10) : ListSequence(max)
    {
    ...
    ListSequence(List<Type> &list, int max = 10) : ListSequence(&list, max)
    {
    }
